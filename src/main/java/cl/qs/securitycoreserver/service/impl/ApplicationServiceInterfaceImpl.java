...
public class ApplicationServiceInterfaceImpl implements ApplicationServiceInterface {
  ...
  @Override
  public List<ApplicationResponseDto> findBy(ApplicationRequestDto applicationRequestDto)
  ...
  public ApplicationResponseDto save(ApplicationRequestDto applicationRequestDto)
      throws SecurityCoreServerException {
    Optional<Application> applicationOptional = applicationRepository.findByName(
        applicationRequestDto.getName());
    if (applicationOptional.isEmpty()) {
      return ApplicationMapper.buildApplicationResponse(
          applicationRepository.save(ApplicationMapper.buildApplication(applicationRequestDto)));
    } else {
  ...
  @Override
  public ApplicationResponseDto update(ApplicationRequestDto applicationRequestDto)
      throws SecurityCoreServerException {
    Optional<Application> applicationOptional = applicationRepository.findById(
        applicationRequestDto.getId());

    if (applicationOptional.isPresent()) {
      Application application = ApplicationMapper.buildApplicationUpdate(applicationRequestDto,
